...
class StrelkaTier2Caller(Caller):
    ...
    def run_caller(self, sample_bam, control_bam, use_cached=True):
        outfolderName = re.sub(r".bam$", ".caller.strelkaTier2", sample_bam)
        ...
        outfileName = os.path.join(outfolderName,"strelkaTier2")
    ...
    def get_results(self, sample_bam, control_bam, use_cached=True):
        ...
        for variant in vcf:
            ...
            sim_alt_depth = int(variant.format('TIR')[sim_sample_index][1])
            sim_ref_depth = int(variant.format('TAR')[sim_sample_index][1])

            ctl_alt_depth = int(variant.format('TIR')[ctl_sample_index][1])
            ctl_ref_depth = int(variant.format('TAR')[ctl_sample_index][1])
        ...
        for variant in vcf:
            ...
            sim_alt_depth = int(variant.format(alt_base +"U")[sim_sample_index][1])
            sim_ref_depth = int(variant.format(ref_base +"U")[sim_sample_index][1])

            ctl_alt_depth = int(variant.format(alt_base +"U")[ctl_sample_index][1])
            ctl_ref_depth = int(variant.format(ref_base +"U")[ctl_sample_index][1])
